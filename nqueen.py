#N-Queens Problem. N - Queens problem is to place n - queens in
#  such a manner on an n x n chessboard that no queens attack 
...
class gameplay:
    def __init__(self,row,col,width,total_rows):
        #self represents instance
        #__intit__ represents constructor..its caleed everytime when object is created from a class
...
def algorithm(grid):
    sol = solveNQ(lambda: draw(WIN, grid, ROW, WIDTH), grid, 0)
    if not sol:
        print("No Solution")
        return False
    else:
...
def solveNQ(draw, grid, col):

    #base case: if all queens placed return true
    if col >= ROW:
        return True

    #consider tis column and try placing the queen in all rows
    #one by one
    ...
    for i in range(ROW):
        if isSafe(draw, grid, i, col):
            ...
            if solveNQ(draw,grid, col + 1) == True:
                return True
